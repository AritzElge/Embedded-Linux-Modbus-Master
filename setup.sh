...
"$SCRIPTS_DIR/copy_init_scripts.sh" "$PROJECT_ROOT" || exit 1

echo "Copying .config to buildroot directory..."
cp "./.config" "$BUILDROOT_DIR/.config" || exit 1
...
"$SCRIPTS_DIR/compile_daemons.sh" "$PROJECT_ROOT"  || exit 1

echo "STEP 3: Copying daemons to overlay..."
# Pass PROJECT_ROOT to the daemon copy script
"$SCRIPTS_DIR/copy_daemons.sh" "$PROJECT_ROOT"  || exit 1
